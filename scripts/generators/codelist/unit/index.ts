...
const source = path.resolve(__dirname, "./codelist/unit/output.xml");

export const unit = defineParser(async (ctx) => {
	...
	return {
		packageRoot: "./packages/codelists/unit",
		data: entries.map(({ "@source": source, ...entry }) => ({
			...entry,
			source,

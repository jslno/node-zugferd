...
const levelColors: Record<LogLevel, string> = {
	...
	info: colors.bg.blue,
	warn: colors.bg.yellow,
	error: colors.bg.red,
	debug: colors.bg.magenta,
};

function formatMessage(level: LogLevel, message: string): string {
	const timestamp = new Date().toISOString();
	return `${colors.dim}${timestamp}${colors.reset} ${colors.bright}${levelColors[level]} ${level.toUpperCase()} ${colors.reset} ${colors.bright}[node-zugferd]:${colors.reset} ${message}`;
}

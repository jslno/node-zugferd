...
export const NavLink = ({ href, children, className, external }: Props) => {
	...
	const isActive =
		segment === href.slice(1) || (segment === "(home)" && href === "/");
	...
	return (
		<li>
			<Link
				href={href}
				className={cn(
					"w-full h-9 flex items-center px-5 transition-colors rounded-lg",
...
export const NavBar = () => {
	...
	return (
		<div className="flex flex-col sticky top-0 bg-background/50 z-30">
			<nav className="md:h-[58px] md:border-b top-0 flex items-center">
				<Link
					href="/"
				...
				<div className="w-full flex items-center justify-end relative gap-8 px-4">
					<div
						className="cursor-text w-full max-w-72 flex items-center gap-2 p-2 px-4 border border-input h-9 rounded-lg"
						onClick={() => {
							setOpenSearch(true);
						}}
					>

...
<Callout type="warn">Templates are rendered server-side only and
do not support streaming or waiting for data.</Callout>
...
```ts title="invoicer.ts"
import { renderer } from "@node-zugferd/api/react/renderer";
import { Template } from "./your/path/template";
